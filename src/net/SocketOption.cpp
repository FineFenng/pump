...
SOCKET SocketOpen(int family, int type, int protocol) {
  int tmp_protocol = protocol;
#ifdef PUMP_PLATFORM_WIN
  WSADATA wsa;
  if (WSAStartup(MAKEWORD(2, 2), &wsa) != 0) { LOG_FALAL << "Windows socket init failed"; }
  if (type == SOCK_STREAM) {
    tmp_protocol = IPPROTO_TCP;
  }
  if (type == SOCK_DGRAM) {
    tmp_protocol = IPPROTO_UDP;
  }
#endif
  return socket(family, type, tmp_protocol);
}
...
/*
* whether in blocking or non-blocking model,
* error num will be set, if errno happened.
*/
